...
samples="${ELS_SAMPLES:-Blastula Blood}"
...
[ -f "${tss}" ] || { echo "Missing TSS BED: ${tss}" >&2; exit 1; }
[ -f "${prox}" ] || { echo "Missing proximal BED: ${prox}" >&2; exit 1; }
[ -f "${rdhs}" ] || { echo "Missing rDAR BED: ${rdhs}" >&2; exit 1; }
