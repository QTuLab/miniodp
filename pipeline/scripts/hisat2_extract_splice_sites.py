...
def extract_splice_sites(gtf_file, verbose=False):
    ...
    if verbose:
        ...
        print(
            "genes: {}, genes with multiple isoforms: {}".format(
                len(genes), sum(len(v) > 1 for v in genes.values())
            ),
            file=stderr,
        )
        print(
            "transcripts: {}, transcript avg. length: {:.0f}".format(
                len(trans), sum(trans_lengths.elements()) // len(trans)
            ),
            file=stderr,
        )
        print(
            "exons: {}, exon avg. length: {:.0f}".format(
        ...
        print(
            "introns: {}, intron avg. length: {:.0f}".format(
        ...
        print(
            "average number of exons per transcript: {:.0f}".format(
...
if __name__ == "__main__":
    parser = ArgumentParser(description="Extract splice junctions from a GTF file")
    parser.add_argument(
        "gtf_file",
        nargs="?",
        type=FileType("r"),
